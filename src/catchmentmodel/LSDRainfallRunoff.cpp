    ...
    for (int n=1; n<=jmax; n++)
    {
      ...
      if (elevations[m][n] > -9999)
      {
        ...
        if(pet_grid != nullptr)
        {
          ...
          double j_old = new_j_mean_array[m][n];
          new_j_mean_array[m][n] -= pet_value; //pet is m/second, j should also be m/second??
          if(pet_value > 0 && j_old > 0)
            std::cout << "Remove PET at rate " << pet_value << " new j " << new_j_mean_array[m][n] << " old j " << j_old << std::endl; 
        }
